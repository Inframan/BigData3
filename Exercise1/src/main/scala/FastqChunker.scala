...
object FastqChunker 
{
	def main(args: Array[String]) 
	{
		...
		//readConf.set("textinputformat.record.delimiter", "\n\n")
		readConf.set("textinputformat.linespermap", "4")
		...
		val first_avroRdd = sc.newAPIHadoopFile(args(1) + "/fastq1.fq", classOf[TextInputFormat], classOf[LongWritable], classOf[Text], readConf).setName("rdd_first_read")

		first_avroRdd.take(10).foreach(rdd => println(rdd._2.toString.split("\n").length +": " + rdd._2))
