	...
	Array[(Int, SAMRecord)] = 
{
	...
	val outFileName = config.getInputFolder + x + ".output"
...
//	println(initialRdd.partitions.size)
    ...
    val files = folder.listFiles.filter(_.getName.endsWith(".fq.gz")).toList

    val arraynerino: ListBuffer[Array[(Int, SAMRecord)]] = new ListBuffer[Array[(Int, SAMRecord)]]()

    for(file <- files)
    	arraynerino += bwaRun(file.getName, config)

    arraynerino.foreach(x => println("int: "+x(1)+ " rest: " + x(2)))
